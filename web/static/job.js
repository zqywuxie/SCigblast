(() => {
  ...
  function syncSelection(){
    const confirmButton=$('[data-action="confirm-match"]');
    if(confirmButton){confirmButton.disabled=tab==='match'&&(matchLoading||batchBusy||!reviewCounts||reviewCounts.unmarked>0);confirmButton.title=tab==='match'&&reviewCounts?.unmarked?`还有 ${reviewCounts.unmarked} 条未标注`:'';}
