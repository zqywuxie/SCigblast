...
window.Submission = (() => {
  ...
  function open(initial, accept) {
    ...
    modal.innerHTML = `<header><div><h2>检查样本与 Note 路径</h2></div><button class="close-button" data-close aria-label="关闭">×</button></header><p>只修改工作副本。合并单元格的 Note 会应用到对应的所有样本。灰色字段仅供查看。</p><div class="editor-toolbar"><select aria-label="工作表" data-sheet></select><a data-download>下载当前 XLSX</a><a data-original>下载原始副本</a></div><div class="editor-toolbar"><input data-old placeholder="原 Note 路径前缀" aria-label="原路径前缀"><input data-new placeholder="分析服务器路径前缀" aria-label="新路径前缀"><button class="button button-ghost" data-replace>预览并替换 Note 前缀</button></div><div data-grid class="table-wrap editor-grid"></div><div class="pagination"><button class="button button-ghost" data-prev>上一页</button><span data-page></span><button class="button button-ghost" data-next>下一页</button></div><p data-message role="status"></p><footer><span data-revision></span><button class="button button-primary" data-save>保存工作副本并使用</button></footer>`;
