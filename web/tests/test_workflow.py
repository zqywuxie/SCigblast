...
class WorkflowTests(unittest.TestCase):
    ...
    def test_unmarked_rows_block_confirmation_across_pages_filters_and_revisions(self):
        ...
        pending = self.client.get(f'/api/jobs/{jid}/match-preview?unmarked_only=true').json()
        self.assertEqual(pending['total'], 1)
        self.assertEqual(pending['rows'][0]['_row_key'], '150')
        self.assertEqual(pending['next_unmarked'], {'row_key':'150', 'offset':150})
        self.assertEqual(pending['review_counts']['total'], 151)
        denied = confirm()
    ...
    def test_unique_default_output_names(self):
        ...
        self.assertEqual(path.parent, self.out/'郑钦云_testadmi'/'igblast_base')
        self.assertRegex(path.name, r'^\d{8}_\d{6}$')
        self.assertEqual(first.json()['operator'], '郑钦云')
        self.assertEqual(web.default_job_output({'display_name':'ZQY','id':'123456789'}, 'ir_split').parent, self.out/'ZQY_12345678'/'ir_split')
        self.assertEqual(web.default_job_output({'display_name':'User Name','id':'876543210'}, 'ir_split').parent, self.out/'User_Name_87654321'/'ir_split')
    ...
    def test_user_pipeline_time_layout_reaches_each_runner(self):
        for pipeline in web.REGISTRY:
            with self.subTest(pipeline=pipeline):
                ...
                self.assertEqual(output.parent, self.out/'郑钦云_testadmi'/pipeline)
