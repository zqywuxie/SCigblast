...
部署时服务自动创建首个管理员，可直接登录。密码为 12–128 字符；显示名称为可选英文姓名，留空使用用户名。密码建议用单引号包裹，避免 Compose 展开其中的 `$`。实际密码只放在部署配置中，不提交进 Git；镜像不包含默认密码。
...
手动创建时密码通过隐藏提示输入两次（12–128 字符），不要写在命令行或提交进 Git。
