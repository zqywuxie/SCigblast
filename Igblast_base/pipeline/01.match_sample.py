#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Create the baseline file-to-sample/Chain mapping.
...
The generated CSV is the auditable contract consumed by the baseline fastp,
clean, PANDAseq and IgBLAST stages.  Barcode columns remain in the common
schema for compatibility, but baseline mode leaves them empty.
"""
...
SUBMISSION_VALUE = (os.environ.get("SCIGBLAST_MATCH_SUBMISSION") or
                    os.environ.get("SCIGBLAST_BASE_SUBMISSION") or
                    os.environ.get("SCIGBLAST_SUBMISSION_XLSX") or "")
SUBMISSION_PATHS_VALUE = os.environ.get("SCIGBLAST_SUBMISSION_PATHS", "")
SUBMISSION_XLSX = Path(SUBMISSION_VALUE)
BARCODE_CSV = Path(os.environ.get("SCIGBLAST_BARCODE_CSV", ""))
# Baseline does not perform barcode splitting.  Keep barcode fields in the
# common manifest schema, but do not require/resolve a barcode reference.
NO_BARCODE = os.environ.get("SCIGBLAST_MATCH_NO_BARCODE", "1") == "1"
INPUT_VALUE = (os.environ.get("SCIGBLAST_MATCH_INPUT") or
               os.environ.get("SCIGBLAST_BASE_RAW_INPUT") or
               os.environ.get("SCIGBLAST_RAW_INPUT_DIR") or "")
INPUT_DIR = Path(INPUT_VALUE)
OUTPUT_SUMMARY = Path(os.environ.get(
    "SCIGBLAST_MATCH_OUTPUT",
    str(OUTPUT_ROOT / "01.match" / "sample_manifest.csv"),
))
SHEET: str | None = None
...
def load_metadata_collection(path: Path | list[Path], sheet: str | None, sample_column: str,
    ...
    """Load one workbook or merge all workbooks under a submission directory.

    Note-based matching is performed after records from every workbook are
    combined; workbook filenames are never used to select a dataset.
    """
