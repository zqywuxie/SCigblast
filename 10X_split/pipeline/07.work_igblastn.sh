...
DATASET_LABEL="${RUNTIME_DATASET_LABEL:-}"
...
MERGED_DIRS=(
    "$(stage_root 06.split_output)/representative_fasta"
)        # 10X representative FASTA directory
INPUT_MODE="representative"                                      # representative | pandaseq
DATA_DIR_SET=0
OUTPUT_DIR="${SCIGBLAST_IGBLAST_OUTPUT_DIR:-$(stage_root 07.igblastn_out)}"             # igblastn output directory
...
MONITOR_DIR=""
SUMMARY_FILE=""
CHAIN_SUMMARY="${SCIGBLAST_10X_CHAIN_SUMMARY:-$(stage_root 01.match)/sample_barcode_summary.csv}"
...
chain_allowed_for_sample() {
    local sample="$1" chain="$2" known chains
    [[ "${#SAMPLE_CHAIN_MAP[@]}" -gt 0 ]] || return 0
    for known in "${!SAMPLE_CHAIN_MAP[@]}"; do
        case "$sample" in
            "$known"|"$known"__*|*__"$known"|*__"$known"__*)
                chains=",${SAMPLE_CHAIN_MAP[$known]},"
                [[ "$chains" == *",$chain,"* ]] && return 0
                return 1
                ;;
        esac
    done
...
case "$INPUT_MODE" in
    ...
    pandaseq)
        if [ "$DATA_DIR_SET" -eq 0 ]; then
            MERGED_DIRS=("${OUTPUT_ROOT}/06.split_output/representative_fasta")
        fi
        ;;
...
Options:
  ...
  --output-dir DIR            igblastn 输出目录，默认 ${SCRIPT_DIR}/6.igblastn_out
...
BATCH_MANIFEST_FINGERPRINT="${BATCH_TEMP_DIR}/manifest.fingerprint"
CONFIG_FINGERPRINT="$(printf '%s\n' "$INPUT_MODE|$DB_DIR|$SPECIES|$db_cell_type|$chains_input|$bcr_chains_input|$BATCH_FASTA" | sha256sum | awk '{print $1}')"
...
MANIFEST_VALID=0
MANIFEST_ROWS=0
if [ -s "$BATCH_MANIFEST" ] && [ -s "$BATCH_MANIFEST_FINGERPRINT" ] && [ "$(cat "$BATCH_MANIFEST_FINGERPRINT")" = "$CONFIG_FINGERPRINT" ]; then
    MANIFEST_VALID=1
    while IFS=$'\t' read -r _fa _mp _sm _sd; do
        ...
        if [ "$INPUT_MODE" = "pandaseq" ]; then
            ...
            # Representative mode must have a non-empty map.  Accepting an
            # empty legacy map here causes post-processing to report a
            # confusing "representative map missing/empty" error after all
            # expensive IgBLAST work has completed.
            [[ "$_mp" != "-" && -s "$_mp" ]] || { MANIFEST_VALID=0; break; }
        fi
    done < "$BATCH_MANIFEST"
    (( MANIFEST_ROWS > 0 )) || MANIFEST_VALID=0
fi

if [ -d "$BATCH_TEMP_DIR" ] && [ -f "$BATCH_MANIFEST" ] && [ "$MANIFEST_VALID" -eq 1 ]; then
    ...
    : > "$BATCH_MANIFEST"
    printf '%s\n' "$CONFIG_FINGERPRINT" > "$BATCH_MANIFEST_FINGERPRINT"
    ...
    for MERGED_DIR in "${MERGED_DIRS[@]}"; do
        ...
        # Discover leaf sample directories recursively so split outputs can
        # retain Lane/batch parent paths without being mistaken for samples.
        SUBDIRS=()
        ...
        if [ "${#SUBDIRS[@]}" -eq 0 ]; then
            ...
            while IFS= read -r -d '' file; do
                BATCH_FASTAS+=("$file")
                BATCH_MAPS+=("")
                BATCH_SAMPLES+=("$(basename "$file")")
                BATCH_SUBDIRS+=("$input_base")
                printf '%s\t%s\t%s\t%s\n' "$file" "-" "$(basename "$file")" "$input_base" >> "$BATCH_MANIFEST"
            ...
            for subdir in "${SUBDIRS[@]}"; do
                ...
                while [ "$batch_start" -lt "${#BC_FILES[@]}" ]; do
                    ...
                    # Validate each representative map once after the batch
                    # is complete.  Performing this inside the input-file
                    # loop repeatedly rescanned an ever-growing map and made
                    # large batches needlessly slow; it also obscured which
                    # batch was malformed.  Pandaseq batches intentionally do
                    # not have a map.
                    if [ "$INPUT_MODE" = "representative" ]; then
                        if [ ! -s "$batch_map" ]; then
...
for ct in "${CELL_TYPES[@]}"; do
    ...
    for chain_name in "${JOB_CHAINS_COUNT[@]}"; do
        for sample_name in "${BATCH_SAMPLES[@]}"; do
            chain_allowed_for_sample "$sample_name" "$chain_name" && TOTAL_TASKS=$((TOTAL_TASKS + 1))
        done
    done
done
DONE_TASKS=0
...
for DB_CELL_TYPE in "${CELL_TYPES[@]}"; do
    ...
    for CHAIN in "${JOB_CHAINS[@]}"; do
        ...
        for ((bi=0; bi<TOTAL_BATCHES; bi++)); do
            ...
            if ! chain_allowed_for_sample "$SAMPLE" "$CHAIN"; then
            ...
            # In representative mode the source map is kept beside the
            # source batch FASTA (one map per sample-local batch), while
            # IgBLAST results are written below <sample>/<chain>.  Publish a
            # cheap symlink at the result location so post-processing can
            # resolve the map by the result batch path.  The postprocessor
            # also has a legacy sibling-map fallback for batches created
            # before this fix.
            ...
            (
                ...
                if [ "$EXIT_CODE" -eq 0 ]; then
                    ...
                    log "${TASK_LABEL} failed with exit code ${EXIT_CODE} after ${ELAPSED_SEC}s"
                fi
            ) &
            active_jobs=$((active_jobs + 1))
        done
...
# Publish only filtered, sample-local results.  Representative IDs are
# replaced with barcode values by the branch-local post-processor.  Raw
# IgBLAST files are temporary and are discarded after atomic publish.
...
def load_map(path):
    # New batches expose a symlink/copy beside batch_*.tsv.  Older batches
    # kept maps beside the sample-local batch FASTA instead, and used a local
    # batch index while the IgBLAST result uses the global batch index.  In
    # that legacy layout load all sibling maps from the sample directory so
    # the sequence header can still be resolved without rebuilding IgBLAST.
    candidates=[path] if path.is_file() else []
    if not candidates:
        sample_dir=path.parent.parent
        candidates=sorted(sample_dir.glob('batch_*.map'))
    if not candidates:
        raise ValueError(f"representative map missing: {path}")
    d={}
    for candidate in candidates:
        for line in candidate.read_text(encoding='utf-8',errors='replace').splitlines():
            k,sep,v=line.partition('\t')
            if sep:
                if k in d and d[k]!=v: raise ValueError(f"conflicting representative map entry: {k}")
                d[k]=v
    # A stale/empty result-side map should not hide valid source maps.
    if not d and candidates == [path]:
        candidates=sorted(path.parent.parent.glob('batch_*.map'))
        for candidate in candidates:
            for line in candidate.read_text(encoding='utf-8',errors='replace').splitlines():
                k,sep,v=line.partition('\t')
                if sep:
                    ...
                    d[k]=v
    if not d:
        raise ValueError(f"representative map empty: {path}")
    return d
...
for batch in files:
    rel=batch.relative_to(root); chain=rel.parent.name.upper(); sample=rel.parent.parent.as_posix(); mp=None; raw=valid=prod=kept=0; errors=[]; header=None; saw_noncomment=False; empty_result=False
    try:
        ...
        with batch.open(encoding='utf-8',errors='replace') as fh:
            for line in fh:
                ...
                fields=line.rstrip('\r\n').split('\t')
                if header is None:
                    if ('sequence_id' not in fields and 'barcode' not in fields) or 'v_call' not in fields:
                        raise ValueError(f'missing AIRR header: {batch}')
                    # 10X publishes the cell identity under the explicit
                    # barcode name.  Keep the raw query ID in the row until
                    # the representative map is applied, then emit barcode.
                    if mode == 'representative' and 'sequence_id' in fields:
                        fields[fields.index('sequence_id')] = 'barcode'
                    header=fields; continue
                if len(fields)<len(header): fields += ['']*(len(header)-len(fields))
                raw+=1; si=header.index('barcode' if 'barcode' in header else 'sequence_id'); vi=header.index('v_call'); pi=header.index('productive') if 'productive' in header else -1; vv=fields[vi].strip().upper() not in bad; pp=pi>=0 and fields[pi].strip().upper() in {'T','TRUE'}
                valid += int(vv); prod += int(pp)
                if not (vv and pp): continue
                if mode=='representative':
                    if mp is None: mp=load_map(batch.with_suffix('.map'))
                    barcode=mp.get(fields[si],'')
                    if not barcode: errors.append(f'missing representative map entry: {fields[si]}'); continue
                    fields[si]=barcode
        ...
        if header is None or raw == 0:
            # Empty/comment-only output is expected when a scheduled chain
            # has no input sequences (some versions still emit only a header).
            # It is not a malformed AIRR file.
            empty_result=True
...
log "Published filtered sample-local TCR.tsv/BCR.tsv; raw IgBLAST files were discarded."
